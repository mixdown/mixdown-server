...
Config.prototype.env = function(env) {
	...
	if (env) {
		...
		_.bind(mergeSection, env.app, this.config.app, 'plugins')();

		_.each(this.config.sites, function(site) {
			var child = _.find(env.sites, function(s) { return s.id == site.id; });

			if (child) {
				// apply overrides of plugins
				_.bind(mergeSection, child, site, 'plugins')();

				// apply overrides of hostmap
				site.hostmap = child.hostmap;
			}
		});
